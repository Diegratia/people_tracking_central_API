...
namespace CentralMonitoring.Presentation.Controllers
{
    ...
    public class FloorplanController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetFloorplans(CancellationToken cancellationToken)
        {
            var sites = new List<Site>();
            _configuration.GetSection("Sites").Bind(sites);

            var activeSites = sites.FindAll(s => s.IsEnabled);
            var results = await _aggregatorService.GetFlatFloorplansAsync(activeSites, cancellationToken);
